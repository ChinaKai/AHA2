...
如果来源设备已经永久停用或数据被清空，它遗留的只读 Task 可以在任务列表或详情页选择“移除孤立任务”。该操作需要再次确认，会按原来源设备和 Task 身份生成持久化删除标记、移除本机镜像及其硬件凭据，并立即尝试同步。其他设备会在下一次同步时移除同一镜像；新设备全量回放也不会将其复活。不要用此操作删除仍在使用的来源设备数据，否则来源设备再次同步时可能重新发布该 Task。
