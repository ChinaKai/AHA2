...
arch="${Arch:-}"
input_exe="${InputExe:-}"
output_dir="${OutputDir:-${repo_dir}/dist/linux}"
validate_only="${ValidateOnly:-false}"

usage() {
    cat <<'EOF'
Usage: scripts/build-linux-packages.sh --version VERSION --arch amd64|arm64 \
...
if [[ -z "$input_exe" || ! -f "$input_exe" ]]; then
    echo "InputExe must name an existing AHA2 Linux binary." >&2
    exit 2
fi

required_files=(
    "${package_dir}/aha2.service"
    "${package_dir}/aha2.env"
...
grep -Fq 'User=aha2' "${package_dir}/aha2.service"
grep -Fq 'Group=aha2' "${package_dir}/aha2.service"
grep -Fq 'EnvironmentFile=/etc/aha2/aha2.env' "${package_dir}/aha2.service"
grep -Fq 'ExecStart=/usr/bin/aha2 serve' "${package_dir}/aha2.service"
grep -Fq 'Restart=on-failure' "${package_dir}/aha2.service"
grep -Fxq 'AHA2_LISTEN=127.0.0.1:8766' "${package_dir}/aha2.env"
grep -Fxq 'AHA2_DATA_DIR=/var/lib/aha2' "${package_dir}/aha2.env"
grep -Fxq '/etc/aha2/aha2.env' "${package_dir}/debian/conffiles"
grep -Fq '%config(noreplace) /etc/aha2/aha2.env' "${package_dir}/rpm/aha2.spec.in"

mkdir -p "$output_dir"
output_dir="$(cd "$output_dir" && pwd)"
input_exe="$(cd "$(dirname "$input_exe")" && pwd)/$(basename "$input_exe")"
deb_output="${output_dir}/aha2_${version}_${deb_arch}.deb"
rpm_output="${output_dir}/aha2-${version}-1.${rpm_arch}.rpm"

echo "validated AHA2 Linux package inputs: version=${version} deb_arch=${deb_arch} rpm_arch=${rpm_arch}"
...
install -d "${deb_root}/DEBIAN" "${deb_root}/usr/bin" "${deb_root}/etc/aha2" "${deb_root}/lib/systemd/system"
install -m 0755 "$input_exe" "${deb_root}/usr/bin/aha2"
install -m 0644 "${package_dir}/aha2.env" "${deb_root}/etc/aha2/aha2.env"
install -m 0644 "${package_dir}/aha2.service" "${deb_root}/lib/systemd/system/aha2.service"
...
if command -v rpmbuild >/dev/null 2>&1; then
    ...
    install -m 0755 "$input_exe" "${rpm_root}/SOURCES/aha2"
    install -m 0644 "${package_dir}/aha2.service" "${rpm_root}/SOURCES/aha2.service"
    install -m 0644 "${package_dir}/aha2.env" "${rpm_root}/SOURCES/aha2.env"
    sed -e "s/@VERSION@/${version}/g" -e "s/@RPM_ARCH@/${rpm_arch}/g" "${package_dir}/rpm/aha2.spec.in" > "${rpm_root}/SPECS/aha2.spec"
    rpmbuild -bb --target "$rpm_arch" --define "_topdir ${rpm_root}" "${rpm_root}/SPECS/aha2.spec"
    built_rpm="${rpm_root}/RPMS/${rpm_arch}/aha2-${version}-1.${rpm_arch}.rpm"
    if [[ ! -f "$built_rpm" ]]; then
