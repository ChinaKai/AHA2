...
func TestChannelSemanticEventOnlyProjectsFinalAgentReply(t *testing.T) {
	...
	if _, _, _, _, ok := channelSemanticEvent("task", "agent_message", map[string]any{"text": "draft", "agent_id": "main"}); ok {
		t.Fatal("unclassified agent message would create a duplicate channel reply")
	}
	eventClass, eventType, payload, _, ok := channelSemanticEvent("task", "agent_message", map[string]any{"text": "working", "agent_id": "main", "intermediate": true})
	if !ok || eventClass != "update" || eventType != "agent_message_update" || payload["text"] != "working" {
		t.Fatalf("intermediate projection=%q/%q %#v ok=%v", eventClass, eventType, payload, ok)
	}
