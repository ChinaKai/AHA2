...
- Before the final response of every Turn, perform a Knowledge closeout. Decide whether the Turn produced a durable, reusable project fact, decision, practice, diagnostic, or Agent Lesson; do not end the Turn without making this decision explicit in the work state.
- If the closeout finds durable new or changed material, the Main Agent must submit a Knowledge candidate or revision through the Agent Control API before the final response. Do not wait for a later Turn or rely on merely mentioning the result in chat.
- If an existing entry materially influenced the work, submit helped feedback. If evidence contradicts it, submit stale or wrong feedback and, when the replacement is known, submit a revision candidate. If no durable change is justified, do not invent a candidate; explicitly record that no Knowledge submission is needed.
