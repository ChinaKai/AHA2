...
type backgroundDesktopHTTPFixture struct {
	*desktopStreamFixture
	selected int
}

func (p *backgroundDesktopHTTPFixture) SelectBackgroundWindow(_ context.Context, id string) (desktop.Window, error) {
	p.selected++
	return desktop.Window{ID: id, Kind: "window", DesktopID: "server-bound-other", OtherDesktop: true}, nil
}

func TestBackgroundOtherDesktopRequiresOwnerConsentAndServerBinding(t *testing.T) {
	h := newStreamHarness(t)
	h.manager.Stop(h.taskID, h.session.ID)
	p := &backgroundDesktopHTTPFixture{desktopStreamFixture: h.fixture}
	manager := desktop.New(p)
	t.Cleanup(func() { manager.Close() })
	h.api.desktop = manager
	payload := map[string]any{"mode": "background", "confirm_shared": true,
		"target": map[string]any{"kind": "window", "window_id": "other-window"}}
	response := requestJSON(t, h.client, http.MethodPost, h.base+"/session", payload, "")
	if response.StatusCode != http.StatusForbidden || p.selected != 0 {
		t.Fatal("background selection bypassed CSRF")
	}
	response.Body.Close()
	payload["confirm_shared"] = false
	response = requestJSON(t, h.client, http.MethodPost, h.base+"/session", payload, h.csrf)
	legacy := manager.Status(h.taskID).Session
	if response.StatusCode != http.StatusCreated || p.selected != 1 || legacy == nil || legacy.Controller != "owner" {
		t.Fatal("legacy background selection did not preserve owner-only grant")
	}
	response.Body.Close()
	if _, err := manager.Observe(context.Background(), h.taskID, legacy.ID, "agent:ungranted"); err == nil {
		t.Fatal("unconfirmed background grant authorized Agent access")
	}
	manager.Stop(h.taskID, legacy.ID)
	p.selected = 0
	payload["confirm_shared"] = true
	payload["target"] = map[string]any{"kind": "window", "window_id": "other-window", "desktop_id": "injected"}
	response = requestJSON(t, h.client, http.MethodPost, h.base+"/session", payload, h.csrf)
	if response.StatusCode != http.StatusBadRequest || p.selected != 0 {
		t.Fatal("client desktop binding override reached provider")
	}
	response.Body.Close()
	payload["target"] = map[string]any{"kind": "window", "window_id": "other-window"}
	response = requestJSON(t, h.client, http.MethodPost, h.base+"/session", payload, h.csrf)
	var body struct {
		Status desktop.Status `json:"status"`
	}
	decodeResponse(t, response, &body)
	if response.StatusCode != http.StatusCreated || p.selected != 1 || !body.Status.BackgroundDesktopSupported ||
		body.Status.Session == nil || body.Status.Session.Controller != "shared" ||
		body.Status.Session.Mode != "background" || body.Status.Session.Window.DesktopID != "server-bound-other" {
		t.Fatal("background capability, consent or trusted binding lost", response.StatusCode)
	}
}
